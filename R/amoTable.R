amoTable <- function(data, outcomeVar = "All", stdGroup = "All Students", unit = NULL) {
	...
	if (outcomeVar == "All") {
		...
		dataTable <- tempData %>% rename("School ID" = schid,
										 ...
										 "Reading Proficiency Index" = rlapctgr,
										 "Reading AMO Goals" = rlagoalgr,
										 "Reading Participation Rates" = rlapartgr,
										 "Math Proficiency Index" = mthpctgr,
										 "Math AMO Goals" = mthgoalgr,
										 "Math Participation Rates" = mthpartgr)
